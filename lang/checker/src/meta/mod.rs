...
impl Checker for Standard {
    ...
    fn check(
        ...
        if !metatable.is_metatable {
            return Err(AnalyzeError::InvalidMetatable {
                span: metatable.span,
            });
        }
        ...
        for (key, value) in metatable.entries.iter() {
            if let types::TableFieldKey::Name(name, span) = key {
                if let Some(accepted) = METAMETHODS.get(name) {
                    ...
                    if accepted == &AcceptedType::Table && matches!(value, types::Type::Table(..)) {
                        did_accept = true;
                    }
                    if accepted == &AcceptedType::Function
                        && matches!(value, types::Type::Function(..))
                    {
                        did_accept = true;
                    }
                    if !did_accept {
