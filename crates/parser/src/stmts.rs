...
parser_struct!(ParseDoStmt, ast::DoStmt, |_, state: &ParseState<'a>| {
    ...
    Ok((
        state,
        ast::DoStmt::new(ast::Span::from_two_spans(r#do.span(), end.span()), block),
    ))
});
...
parser_struct!(
    ...
    |_, state: &ParseState<'a>| {
        ...
        Ok((
            state,
            ast::LocalFunction::new(
                ast::Span::from_two_spans(start.span(), body.span()),
                name,
                body,
            ),
        ))
...
>| {
    ...
    Ok((
        state,
        ast::RepeatStmt::new(
            ast::Span::from_two_spans(start.span(), condition.span()),
            block,
            condition,
        ),
    ))
...
parser_struct!(
    ...
    |_, state: &ParseState<'a>| {
        ...
        Ok((
            state,
            ast::WhileStmt::new(
                ast::Span::from_two_spans(start.span(), end.span()),
                condition,
                block,
            ),
        ))
...
>| {
    ...
    Ok((
        state,
        ast::NumericFor::new(
            ast::Span::from_two_spans(for_token.span(), end.span()),
            name,
            Box::new(start),
            Box::new(end_exp),
            step,
    ...
    |_, state: &ParseState<'a>| {
        ...
        Ok((
            ...
            ast::ElseIfClause::new(
                ast::Span::from_two_spans(start.span(), block.span()),
                condition,
                block,
            ),
        ))
...
parser_struct!(ParseBlock, ast::Block, |_, state: &ParseState<'a>| {
    ...
    Ok((
        state,
        ast::Block::new(
            ast::Span::from_two_spans(start_position, end_position),
            stmts,
            last_stmt,
        ),
    ))
