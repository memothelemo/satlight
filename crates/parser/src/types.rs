...
parser_struct!(ParseTypeParameter, ast::TypeParameter, |_, state: &ParseState<'a>| {
    // That (`:` Thing)? = Here?
    let (state, name) = ParseName.parse(state)?;
    let start = name.span().start();
    let end = name.span().end();
    let (state, typ) = if let Ok((new_state, _)) = ParseSymbol(ast::SymbolType::Colon).parse(&state) {
        let (new_state, info) = expect!(&new_state, ParseTypeInfo, "<type>");
        (new_state, Some(info))
    } else {
        (state, None)
    };
    let (state, default, end) = if let Ok((new_state, _)) = ParseSymbol(ast::SymbolType::Equal).parse(&state) {
        let (new_state, default) = expect!(&new_state, ParseTypeInfo, "<type>");
        let span_end = default.span().end();
        (new_state, Some(default), span_end)
    } else {
        (state, None, typ.as_ref().map(|v| v.span().end()).unwrap_or(end))
    };
    Ok((state, ast::TypeParameter::new(
...
parser_struct!(ParseTypeParameters, Vec<ast::TypeParameter>, |_, state: &ParseState<'a>| {
    // That (`:` Thing)? = Here?
    let (state, _) = ParseSymbol(ast::SymbolType::LessThan).parse(state)?;
    let (state, params) = OneOrMorePunctuated(ParseTypeParameter, ParseSymbol(ast::SymbolType::Comma), "<type>".to_string()).parse(&state)?;
