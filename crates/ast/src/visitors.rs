...
pub trait AstVisitor<'a, 'b> {
    type BlockOutput: 'a;
    type LastStmtOutput: 'b;

    fn visit_block(&mut self, node: &Block) -> Self::BlockOutput;

    fn visit_last_stmt(&mut self, node: &Stmt) -> Self::LastStmtOutput;
}

// LIFETIMES

pub trait ExprVisitorLifetime<'a> {
    type Output: 'a;
...
pub trait TypeVisitorLifetime<'a> {
    type Output: 'a;
...
pub trait StmtVisitorLifetime<'a> {
    type Output: 'a;
...
pub trait AstVisitorLifetime<'a, 'b> {
    type BlockOutput: 'a;
    type LastStmtOutput: 'b;

    fn visit_block(&mut self, node: &'a Block) -> Self::BlockOutput;

    fn visit_last_stmt(&mut self, node: &'a Stmt) -> Self::LastStmtOutput;
}
